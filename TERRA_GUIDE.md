...
Required Router totals: **240 unique stops**, advancing **144 story missions, 34 camp jobs, 12 NERO checkpoints, 14 ambush camps, 12 infestation zones, 40 hordes**. Sum OCR goals **and** `split.auto`; counting OCR alone undercounts story missions and the three story hordes. Counter overlaps remain one stop. All Storylines excludes collectibles, trophies, cairns, IPCA, research sites, injectors, plants, and loot. The separate Platinum + 100% route must remain untouched. Speedrun Mode can insert local `customMissions` after a selected stop; these are route-only and never advance or alter the 240 Router totals.

Map overrides are in `assets/games/days-gone/speedrun-route-overrides.json`: `routeId`, ordered split IDs, `coordinates`, `vectors`, and `transitions`. Current vectors store intermediate points by the destination split ID. Transitions hold optional `start`, `end`, `teleport`, and ordered `keyPoints` (`x`, `y`, optional `label`, optional `lootId`) belonging to that split ID. Select a mission, use Mark start / Mark end, then Add key point and click once per point. The selected mission displays S → numbered key points → E in cyan; points can be named, moved, reordered, or removed in the sidebar. Once both ends are marked, Show loot in mission area loads loot around the mission path; clicking a shown item adds it as a named key point and stores its `lootId`. Each change uses the existing browser autosave; Save route changes writes the local override. Missing-location filters now mean missing a mission start or end. Existing objective coordinates and travel vectors remain under Advanced route options. The renderer connects a previous mission's end to the next start, and skips that connection when the previous mission has teleport enabled.
...
- Selected mission start/end and required key points are rendered as a straight-segment guide, not a road-following route. Teleport to next mission is available beside the mission controls. When enabled, selecting either adjacent mission shows a purple dashed end-to-start connector; selecting the outgoing mission also shows a T landing marker at the next mission start. The ordinary travel leg is suppressed.
